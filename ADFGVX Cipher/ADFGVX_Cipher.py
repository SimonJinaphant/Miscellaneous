...
def columnar_transposition(ciphered_letters):
	...
	for count, key_letter in enumerate(key_word):
		new_table[key_letter] = [cipher_k for cipher_v, cipher_k in enumerate(ciphered_letters) 
									if cipher_v % len(key_word) == count]

	for v, k in sorted(new_table.iteritems(), key = lambda(v1, k1): v1):
		final_message.extend(k)
